...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evidently: data drift + performance decay мониторинг")
    parser.add_argument("--reference-path", default="data/processed/credit.csv", help="Эталонный датасет (обычно train)")
    parser.add_argument("--current-path", default="data/drift/current.csv", help="Текущий датасет (псевдо-прод)")
    parser.add_argument("--model-path", default="models/model.joblib", help="Sklearn Pipeline (joblib) для предсказаний")
    ...
    parser.add_argument("--drift-threshold", type=float, default=0.5, help="Порог доли дрейфующих фич для флага dataset drift")
    parser.add_argument("--auc-threshold", type=float, default=0.65, help="Порог ROC-AUC для флага performance decay")
    ...
    if not cur_path.exists():
        raise FileNotFoundError(f"Нет current данных: {cur_path}. Сначала сгенерируй их (scripts/drift/simulate_production_data.py)")
